    ...
    <div class="row">
        ...
        <div  v-if="this.$store.cart !== null">
            <table class="table table-bordered">
                ...
                <tr v-for="(product, index) in this.$store.cart" :key="product.item.id">
                    <td class="text-center"><img class="w-25" v-bind:src="'/images/products/' + product[index].item.image"></td>
                    <td>{{ product[index].item.name }}</td>
                    <td class="text-center"><input type="text" name="count" v-model="product.quantity" class="text-center w-50" v-on:change="poprawIlosc(product.item.id, product.quantity)"></td>
                    <td>{{ product[index].item.price }} zł</td>
                    <td>{{ product[index].item.priceGross }} zł</td>
                    <td>{{ product[index].item.tax }}</td>
                    <td>{{ product[index].item.unit }}</td>
