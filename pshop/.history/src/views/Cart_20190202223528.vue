...
<div id="cartoptions">
    ...
    <div class="row">
        ...
        <div class="row">
            <div class="col">
                <span class="btn btn-danger" v-on:click="clear">usuń wszytko</a>
            </div>
        </div>
    </div>
    <div class="col-3">
        ...
        <div class="row">
            <div class="col">
                <div class="row">
                    <div class="col"><p>Kwota całkowita</div>
                    <div class="col">{{ showCartValue }} zł</div>
                </div>
                <div class="row">
                    <div class="col"><p>Wysyłka</div>
                    <div class="col">25,00 zł</div>
                </div>
                <div class="row">
                    <div class="col"><p>Suma</div>
...
export default {
    ...
    mounted() {
        ...
        axios.get("<?php echo $this->config->domain;?>/api/cart/value")
        .then(function(response){
            options.cartvalue = response.data.lista;
        });
    ...
    methods:{
        getList: function() {
            axios.get("<?php echo $this->config->domain;?>/api/cart/get")
            .then(function(response)
            {
        ...
        clear: function(){
            axios.get("<?php echo $this->config->domain;?>/api/cart/clear")
            .then(function(response)
            {
                options.lista = response.data.lista;
                options.cartvalue = 0;
                ...
                .then(function(response)
                {
                    ...
                    {
                        options.lista = response.data.lista;
                        options.cartcount = response.data.lista.lenght;
                        navbar.cartcount = response.data.lista.length;
                    }
                });
        ...
        remove: function(id){
            axios.get("<?php echo $this->config->domain;?>/api/cart/remove/"+id)
            .then(function(response){
                options.lista = response.data.lista;
        ...
        {
            axios.get("<?php echo $this->config->domain;?>/cart/add/" + id + "/" + count)
            .then(function(response)
            {
                ...
                .then(function(response)
                {
                    ...
                    {
                        options.cartvalue = response.data.lista;
                        navbar.cartvalue = response.data.lista;
                    }
                });
